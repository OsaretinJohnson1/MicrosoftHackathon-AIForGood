  ...
  const middleware = import_middleware_storage.clerkMiddlewareRequestDataStorage.run(import_middleware_storage.clerkMiddlewareRequestDataStore, () => {
    const baseNextMiddleware = (0, import_debugLogger.withLogger)("clerkMiddleware", (logger) => async (request2, event2) => {
      ...
      if (options.contentSecurityPolicy) {
        const { header, nonce } = (0, import_content_security_policy.createCSPHeader)(
          (_a = options.contentSecurityPolicy.strict) != null ? _a : false,
          ((_c = (_b = (0, import_keys.parsePublishableKey)(publishableKey)) == null ? void 0 : _b.frontendApi) != null ? _c : "").replace("$", ""),
          options.contentSecurityPolicy.directives
        );
        (0, import_utils.setHeader)(handlerResult, import_internal.constants.Headers.ContentSecurityPolicy, header);
        if (nonce) {
          (0, import_utils.setHeader)(handlerResult, import_internal.constants.Headers.Nonce, nonce);
        }
