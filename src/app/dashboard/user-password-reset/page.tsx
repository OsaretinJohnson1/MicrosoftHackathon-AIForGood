...
import { Loader2, Send, User, Mail, Phone, Lock, Check, ChevronDown } from "lucide-react"
...
const passwordResetSchema = z.object({
  ...
  notificationType: z.enum(["email", "sms"]),
...
export default function UserPasswordResetPage() {
  ...
  const handleNotificationTypeChange = (value: string) => {
    setFormData(prev => ({ ...prev, notificationType: value as "email" | "sms" }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Format identifier for phone if SMS is selected
    let identifier = formData.identifier
    if (formData.notificationType === "sms") {
      // If the phone number starts with "0", remove it
      const phoneNumber = formData.identifier.startsWith("0") 
        ? formData.identifier.substring(1) 
        : formData.identifier
    ...
    try {
      ...
      if (response.ok) {
        setResult({
          success: true,
          message: data.message || "Password has been reset and sent to the user.",
        })
        toast.success("Password has been reset and sent to the user.")
        // Reset form
        setFormData({
          identifier: "",
          notificationType: "email",
  ...
  return (
    <div className="flex-1 space-y-6 p-6 pt-8 md:p-8">
      ...
      >
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-foreground">User Password Reset</h2>
          <p className="text-muted-foreground">Reset a user's password and send it automatically</p>
        </div>
      </motion.div>
        ...
        >
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enter a user's email or phone number to reset their password and notify them via email or SMS
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  >
                    <div className="flex items-center space-x-2 rounded-md border p-3">
                      ...
                      <Label htmlFor="option-email" className="flex items-center gap-2 cursor-pointer">
                        <Mail className="h-4 w-4" />
                        <span>Email the new password</span>
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2 rounded-md border p-3">
                      <RadioGroupItem value="sms" id="option-sms" />
                      <Label htmlFor="option-sms" className="flex items-center gap-2 cursor-pointer">
                        <Phone className="h-4 w-4" />
                        <span>SMS the new password</span>
                      </Label>
                    </div>
                  </RadioGroup>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-medium">
                      {result.success ? "Password Reset Successful" : "Password Reset Failed"}
                    </h3>
